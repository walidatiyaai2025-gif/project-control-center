...
"policies/GOVERNANCE_LAWS.md","policies/TASK_LIFECYCLE_AND_LEASE.md","policies/CENTRAL_ORCHESTRATION_POLICY.md","policies/PROJECT_FAMILY_ROUTING_POLICY.md","policies/CONSTITUTIONAL_DECISION_AND_VARIANT_ONBOARDING_POLICY.md","policies/EMERGENCY_PRODUCTION_INCIDENT_POLICY.md","policies/IMMUTABLE_PRODUCT_VERSION_POLICY.md","policies/END_TO_END_FEATURE_DELIVERY_POLICY.md","policies/FLEET_CONTROL_POLICY.md","policies/EXECUTION_OUTPUT_DISCIPLINE_POLICY.md",
...
"scripts/version_governance.py","scripts/orchestrator.py","scripts/enrollment_controller.py","scripts/route_work.py","scripts/variant_governance.py","scripts/incident_governance.py","scripts/feature_delivery_audit.py","scripts/github_fleet_client.py","scripts/fleet_control.py","scripts/fleet_readiness.py","scripts/self_protection.py","scripts/output_discipline.py",
"scripts/test_control_plane.py","scripts/test_feature_delivery.py","scripts/test_fleet_control.py","scripts/test_fleet_readiness.py","scripts/test_route_work.py","scripts/test_incident_governance.py","scripts/test_output_discipline.py",
"schemas/project-family.schema.json","schemas/production-incident.schema.json","schemas/worker-handoff.schema.json","schemas/qa-handoff.schema.json","schemas/ci-handoff.schema.json","schemas/visual-qa-handoff.schema.json","schemas/integration-handoff.schema.json","schemas/release-handoff.schema.json",
"templates/PROJECT_PROFILE.yml","templates/PROJECT_ROUTING.json","templates/PROJECT_FAMILY.json","templates/PRODUCTION_INCIDENT.json","templates/MANAGED_REPOSITORY_CONTROL.yml",
...
contain("AGENTS.md",["MANAGER_LEAD_CONTRACT_VERSION: 1.2.0","Constitutional persistence law","Automatic onboarding variant normalization","Emergency production incident law","SERVICE_RESTORED_TEMPORARY != DONE","PCC ROUTING PACKET","No delegation of ambiguity","Replacement Managers/Leads"])
...
contain("policies/GOVERNANCE_LAWS.md",["NO IMPLEMENTATION DISPATCH WITHOUT AN AUTHORITATIVE PCC ROUTING PACKET","DURABLE OPERATIONAL DECISIONS MUST BE PERSISTED","ADDING/ONBOARDING A PROJECT AUTOMATICALLY REQUIRES PCC MANAGER CLASSIFICATION","VARIANT IMPLEMENTATION WRITES REQUIRE `ROUTING_STATE=READY`","SERVICE_RESTORED_TEMPORARY != DONE","UNRESOLVED TEMPORARY MITIGATIONS MUST BE CARRIED FORWARD"])
...
contain("policies/EMERGENCY_PRODUCTION_INCIDENT_POLICY.md",["POLICY_ID: EMERGENCY_PRODUCTION_INCIDENT_POLICY","SERVICE_RESTORED != INCIDENT DONE","Permanent-fix debt is mandatory","Future-release accounting","Regression protection","Replacement Manager / Lead law"])
...
contain("scripts/incident_governance.py",["SERVICE_RESTORED_TEMPORARY","PERMANENT_FIX_TASK_REQUIRED","TEMPORARY_MITIGATION_MUST_CARRY_FORWARD_UNTIL_PERMANENT_RESOLUTION","CLOSED_REQUIRES_PERMANENT_FIX_SHA"])
contain("scripts/output_discipline.py",["validate_handoff","unsupported DONE","artifact SHA does not match exact HEAD","stale/unverified visual artifact cannot produce authoritative QA","contradictory CI states","open production incidents require incident carry-forward accounting"])
contain("schemas/release-handoff.schema.json",["OPEN_PRODUCTION_INCIDENTS","INCIDENT_CARRY_FORWARD"])
contain("schemas/task.schema.json",["EMERGENCY_MITIGATION","PERMANENT_INCIDENT_FIX","INCIDENT_ID","MUST_CARRY_FORWARD"])
...
incident_template=json.loads((root/'templates/PRODUCTION_INCIDENT.json').read_text(encoding='utf-8'))
incident_template_check=validate_incident(incident_template)
# Template placeholders are intentionally incomplete, but structural emergency defaults must remain.
incident_template_contract=(incident_template.get('PERMANENT_FIX_REQUIRED') is True and incident_template.get('REGRESSION_TEST_REQUIRED') is True and incident_template.get('CARRY_FORWARD_TO_FUTURE_RELEASES') is True and incident_template.get('RELEASE_GATE_STATE')=='OPEN')

status_checks={
"MANAGER_LEAD_CONSTITUTION":has("AGENTS.md","MANAGER_LEAD_CONTRACT_VERSION: 1.2.0","PCC ROUTING PACKET","No delegation of ambiguity") and has("policies/CENTRAL_ORCHESTRATION_POLICY.md","Manager / Lead controller contract"),
...
"EMERGENCY_PRODUCTION_INCIDENT_GOVERNANCE":incident_template_contract and has("AGENTS.md","Emergency production incident law","SERVICE_RESTORED_TEMPORARY != DONE") and has("policies/EMERGENCY_PRODUCTION_INCIDENT_POLICY.md","Permanent-fix debt is mandatory","Future-release accounting") and has("scripts/incident_governance.py","CLOSED_REQUIRES_PERMANENT_FIX_SHA") and has("schemas/release-handoff.schema.json","OPEN_PRODUCTION_INCIDENTS","INCIDENT_CARRY_FORWARD"),
